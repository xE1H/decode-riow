...
import static org.firstinspires.ftc.teamcode.auto.sample.PointsSample.BUCKET_HIGH_SCORE_POSE_TELEOP;
...
public class DriveToBucketTeleop extends SequentialCommandGroup {
    private final Logger logger = Logger.getLogger("Drive to bucket teleop");
                    ...
                    public void run() {
                        ...
                        if (currentPose.getX() > xBoundLow){
                            if (currentPose.getY() > yHighBound){
                                logger.info("robot is in the left sub sector");
                                ...
                                driveToBucketCommand.addCommands(
                                        ...
                                        new FollowPath(follower, bezierPath(target, BUCKET_HIGH_SCORE_POSE_TELEOP).setZeroPowerAccelerationMultiplier(5.5)
                                                .setLinearHeadingInterpolation(target.getHeading(), BUCKET_HIGH_SCORE_POSE_TELEOP.getHeading()).build())
                                );
                            }

                            else if (currentPose.getY() < yLowBound){
                                logger.info("robot is in the right sub sector, skipping");
                            }
                            else{
                            ...
                            Pose finalTarget = new Pose(BUCKET_HIGH_SCORE_POSE_TELEOP.getX() - 2, BUCKET_HIGH_SCORE_POSE_TELEOP.getY() - 4, BUCKET_HIGH_SCORE_POSE_TELEOP.getHeading());
                            ...
                            if (currentPose.getX() > xClose) {
                                driveToBucketCommand.addCommands(
                                        ...
                                        new FollowPath(follower, bezierPath(target, finalTarget).setZeroPowerAccelerationMultiplier(5)
                                                .setLinearHeadingInterpolation(0, finalTarget.getHeading()).build())
                                );
                            }
                            else{
                                driveToBucketCommand.addCommands(
                                        new FollowPath(follower, bezierPath(currentPose, finalTarget).setZeroPowerAccelerationMultiplier(5)
                                                .setLinearHeadingInterpolation(currentPose.getHeading(), finalTarget.getHeading()).build())
                                );
                            }
                        }
                    }
